...
- Two semver prerelease tags are selected, for example `v0.2.0-rc.0` and `v0.2.0-rc.1`.
...
1. Build and publish `v0.2.0-rc.1` as a GitHub prerelease.
...
```bash
pnpm prepare:updater-rehearsal -- --tag v0.2.0-rc.1
pnpm tauri build --config src-tauri/tauri.updater-rehearsal.generated.json
```

Build the source package at version `0.2.0-rc.0`. Never commit the generated overlay and never replace the production endpoint with a prerelease URL.
