...
- 多语言界面：英文、简体中文（稳定）；繁体中文、日文、韩文、俄文、西班牙文（测试版）。
...
Tauri updater 签名（区别于 OS 代码签名）已配置，候选发布流程会校验签名资产；公开发布前仍需完成三平台 `rc.0 → rc.1` 实机升级演练。正式 OS 代码签名将在后续版本中配置，参见 [docs/RELEASE.md](docs/RELEASE.md) 的“当前签名状态”章节。
