...
use russh::{
    client::{self, AuthResult, Config, Handler, Handle},
...
impl SftpEngine {
    ...
    async fn probe_target(&self, pool: &SqlitePool, target: SftpTarget) -> Result<ProbeOutput, String> {
        ...
        let metadata = connection
            ...
            .await
            .map_err(|e| sftp_error("sftp_stat_failed", format!("Could not inspect SFTP file: {e}"), true))?;
        if metadata.is_dir() {
    ...
    async fn run_download(&self, context: DownloadContext) -> Result<(), String> {
        ...
        if let Some(credentials) = db::resolve_task_credentials(&context.pool, &context.task.id).await? {
...
pub async fn probe_sftp_directory_url(
    ...
    let read_dir = connection
        ...
        .await
        .map_err(|e| sftp_error("sftp_directory_probe_failed", format!("Could not list SFTP directory: {e}"), true))?;
...
async fn download_sftp_file(
    ...
    let mut remote = session
        .open(&target.path)
        .await
        .map_err(|e| sftp_error("sftp_open_failed", format!("Could not open SFTP file: {e}"), true))?;
    if resume_from > 0 {
        remote
            .seek(SeekFrom::Start(u64::try_from(resume_from).unwrap_or(0)))
            .await
            .map_err(|e| sftp_error("sftp_resume_failed", format!("Could not seek SFTP file: {e}"), true))?;
    }
    ...
    loop {
        ...
        let read = remote
            .read(&mut buffer)
            .await
            .map_err(|e| sftp_error("sftp_read_failed", format!("SFTP file read failed: {e}"), true))?;
        if read == 0 {
            break;
        }
...
async fn pause_sftp_task(
    ...
    db::update_segment_status(pool, &segment.id, SegmentStatus::Pending, Some(downloaded), None)
        .await?;
    if let Some(current) = db::get_task_record(pool, &task.id).await? {
        emit_task_updated_record(app, pool, &current).await;
    }
...
async fn connect_sftp(
    ...
    let handle_result = if proxy_config.is_custom_socks5() {
        ...
        .await
        .map_err(|e| sftp_error("sftp_proxy_connect_failed", format!("SFTP proxy connection failed: {e}"), true))?;
    ...
    match handle
        ...
        .await
        .map_err(|e| sftp_error("sftp_auth_failed", format!("SFTP password authentication failed: {e}"), true))?
    {
        AuthResult::Success => {}
        AuthResult::Failure { .. } => {
            return Err(sftp_error(
    ...
    let channel = handle
        ...
        .await
        .map_err(|e| sftp_error("sftp_channel_failed", format!("Could not open SFTP channel: {e}"), true))?;
    channel
        .request_subsystem(true, "sftp")
        .await
        .map_err(|e| sftp_error("sftp_subsystem_failed", format!("Could not start SFTP subsystem: {e}"), true))?;
    let session = SftpSession::new(channel.into_stream())
        .await
        .map_err(|e| sftp_error("sftp_subsystem_failed", format!("Could not initialize SFTP session: {e}"), true))?;
    Ok(SftpConnection {
        session,
        _handle: handle,
    })
...
impl SftpTarget {
    ...
    fn parse(input: &str) -> Result<Self, String> {
        ...
        let password = parsed.password().map(percent_decode_lossy).unwrap_or_default();
...
fn sftp_file_url(target: &SftpTarget, path: &str) -> String {
    ...
    if target.port == DEFAULT_SFTP_PORT {
        ...
        format!("sftp://{}{}:{}{}", credentials, target.host, target.port, encoded)
    }
}
...
mod tests {
    ...
    fn sftp_defaults_to_port_22_and_rejects_directories_as_files() {
        let target = SftpTarget::parse_file("sftp://alice:pass@example.com/file.bin")
            .expect("target");
